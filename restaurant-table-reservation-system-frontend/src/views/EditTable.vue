<template>
  <div class="edit-table-form">
    <h1>Edit Table for {{ restaurantName }}</h1>
    ...
    <form @submit.prevent="updateTable">
      ...
      <div class="button-container">
        <button type="submit" class="submit-button">Update Table</button>
    ...
    async fetchTableDetails() {
      try {
        const response = await axios.get(
          `https://restauranttablereservationsystem.azurewebsites.net/api/restaurants/${this.restaurantId}/tables/${this.tableId}`
        );
        this.table = response.data;
        this.restaurantName = localStorage.getItem("currentRestaurantName");
      } catch (error) {
